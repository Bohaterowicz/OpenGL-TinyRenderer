...
vertex_buffer::vertex_buffer(const mesh_data& Data) : GLID(0)
{
	...
	//TODO: Right now I hardcode the buffer size to be 3 position floats, 3 normals floats, and 2 texture coordinates floats (8 together), this could be comptued from data stored in mesh_data in the future
	size_t Count = 8 * Data.Indicies.size() * 3;
	auto DataBuffer = std::make_unique<real32[]>(Count); //new float[Count];
	uint32 DataIndex = 0;
	for (auto& Face : Data.Indicies) {
		for (uint32 TriIndex = 0; TriIndex < 3; ++TriIndex) {
			...
			DataIndex += 3;
			std::memcpy(DataBuffer.get() + DataIndex, glm::value_ptr(Data.TexCoords[Face.tex_coord_idx[TriIndex]]), sizeof(glm::vec2));
			DataIndex += 2;
		}
	}
	GLCall(glGenBuffers(1, &GLID));
