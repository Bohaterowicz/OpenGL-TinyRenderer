...
private:
    ...
    real32 SpecularStrength;

    bool32 UseColorTexture;

public:
    render_material(std::shared_ptr<shader_program> _Shader, glm::vec3 _Color = glm::vec3(0.0F)) : Shader(_Shader), Color(_Color), ColorTexture(nullptr), ColorSampler(nullptr), SpecularStrength(0.0F), UseColorTexture(FALSE) {}
    ...
    inline real32 GetSpecularStrength() const { return SpecularStrength; }
    ...
    inline void SetSpecularStrength(real32 Strength) { SpecularStrength = Strength; }
